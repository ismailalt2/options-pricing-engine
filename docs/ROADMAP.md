...
vol is its inverse, and the tree and Monte Carlo pricers are validated against
it. The IV surface came last since it exercises everything else.

## Checklist

### 1. Black-Scholes — `black_scholes.py`
- [x] `d1(...)` (sanity: canonical set → 0.35)
- [x] `d2(...)` (sanity: canonical set → 0.15)
...
- [x] standard-error calculation → `MCResult`
- [x] `pytest tests/test_monte_carlo.py -v` all green (near BS; se ∝ 1/√n; antithetic reduces variance)
...
- [x] `build_iv_surface(...)` invert each quote → strike×expiry grid
