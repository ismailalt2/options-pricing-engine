...
10th ed. is within a chapter or two if that is what you have). Read the Hull
section for a topic *before* you implement it, do the comprehension questions in
[`NOTES.md`](NOTES.md), then write the code.

Free supplements are chosen to be genuinely useful, not filler:
- **Wikipedia** derivations are surprisingly rigorous for this material and free.
- **QuantStart** has practical Python-flavoured articles on MC and implied vol.
...
- Hull Ch. 5 (forward/futures, no-arbitrage pricing) and Ch. 14 (Wiener
  processes, Itô's lemma) — the machinery under everything else.
...
- Wikipedia: "Greeks (finance)" — it tabulates every closed-form Greek; use it to
  cross-check your formulas (mind the per-unit vs per-1% conventions).

**Answer in NOTES.md before coding**
1. Delta is `∂V/∂S`; derive why call delta = `N(d1)` (differentiate the price and
...
- Hull **Ch. 13** "Binomial Trees" (one- and two-step trees, risk-neutral `p`)
  and **Ch. 21.1–21.5** "Basic Numerical Procedures" (multi-step CRR, backward
...
2. Why does the CRR tree price converge to Black-Scholes as steps → ∞? What is
...
- Hull **21.6** "Monte Carlo Simulation" and **21.7** on variance reduction
  ...
  eq. 21.22.
- QuantStart: "Monte Carlo Simulations In CUDA... " intro parts, and
  "European Vanilla Option Pricing with C++ via Monte Carlo" (ignore the C++;
  read the method and variance-reduction sections).
...
## 6. Market data — `market_data.py` *(loader provided)*

**Read before implementing**
- Nothing new to price here — instead read the provided `market_data.py` end to
  end and the sample CSV in `data/`.
...
3. If the market genuinely obeyed Black-Scholes, what would the surface look like?
   (This is exactly what the flat-surface round-trip test checks.)
